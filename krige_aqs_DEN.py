...
#written by: Kate O'Dell and Will Lassman
...
#pull data for May-Oct of 2015
#tINDS = np.arange(182,305)
#sPM = sPM[tINDS, :]
NT, NS = sPM.shape
...
#vgp = [3.0, 8.0, 0.1]		#Wash 2012 parameters these may need to change for OR 2013, test them
#vgp = [3.0,2.5,0.1]		#Oregon 2013 parameters, tested. 
#vgp = [3.0,2.5,0.1]		#use OR for now, we should probably re-test these, since the front range has a unique 
vgp = [2.8,2.5,0.2]
#monitor set-up
#test variogram parameters
'''
if len(sys.argv) > 1:
...
for si in range(0,NWS):
	...
	#if len(nanINDS) > 0.7 * len(xout):
	#	INVALIDS.append(si)	#sites that don't have a lot of data over the time period .. not
					# an issue now, deal with later when calculating kriging for each 						# day
	#	continue
	xout = np.delete(xout, nanINDS)
	yout = np.delete(yout, nanINDS)
	#kRSQ[si] = np.corrcoef( xout, yout )[1,0]**2.
...
f = open('/home/kaodell/nasa_fires/Martenies_Den3kmKrige/processed_datafiles/kpmDEN2012_3km.npz', 'w')
...
pl.savefig('/home/kaodell/nasa_fires/Sheena_Den3kmKrige/figures/3kmgrid_w_krigesites_CO.png')




############################################################################################################
# FIGURE 2: all PM data going into krige
